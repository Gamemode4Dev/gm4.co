...
window.addEventListener('DOMContentLoaded', () => {
	fetchModulesAndResources()
		...
		.then((promo) => {
			...
			const requiresRecommended = mod.requires.flatMap(r => modules.get(r).recommends).filter(id => id !== loadedModuleId);
...
/**
 * Sets the `currentStep`, then finds an element with the `data-step` attribute set to this step, and scrolls to it.
 ...
 */
 function createCreditsFrame(moduleId) {
	...
	credits = modules.get(moduleId).credits;
